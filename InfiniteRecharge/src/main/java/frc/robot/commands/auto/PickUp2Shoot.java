...
public class PickUp2Shoot extends SequentialCommandGroup {
   ...
   */
  public PickUp2Shoot(ShaftSubsystem shaftClimber, ShooterMain shooterMain,IntakeSubsystem intake, DriveTrainMain driveTrain,
  VisionSystem visionSubsystem, Pose pose) {
    // Add your commands in the super() call, e.g.
    // super(new FooCommand(), new BarCommand());
    super(  new ParallelCommandGroup(new LowerIntake(intake), 
            new LowerShaft(shaftClimber)),
            new ParallelCommandGroup(
            new RamseteCommand(DriveConstants.test, pose::getPose,
    ...
    // RamseteCommand passes volts to the callback
                ...
                new IntakeBalls(intake,.7)
    )
                
            //,

    // new ParallelRaceGroup(new LoadShaftCommand(shaftClimber, 5,shooterMain),
